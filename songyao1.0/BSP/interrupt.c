...
void UPDATA_NODE()
{
    if(pid_lock==0)
    {
        ...
        motor[0].speed=(float)(motor[0].totalAngle-motor[0].lastAngle)*6000/(4*11*30);//算出10ms内的速度r/min
        motor[0].lastAngle=motor[0].totalAngle;
    
        motor[1].totalAngle=pluse2+motor[1].loopnum*arr2/2;//转过总角度=当前计数器值-10000+0*10000	
        motor[1].speed=(float)(motor[1].totalAngle-motor[1].lastAngle)*6000/(4*11*30);
